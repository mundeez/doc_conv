...
def home(request):
    ...
    try:
        ...
        <html><body>
          ...
          <form action="{reverse('md2docx:convert_markdown')}" method="post" enctype="multipart/form-data">
...
def convert_markdown(request):
    ...
    # POST: create a task id and optionally store the uploaded markdown
    task_id = uuid.uuid4()
    saved_path = None

    uploaded = request.FILES.get("file")
    if uploaded:
        dest = UPLOADS_DIR / f"{task_id}.md"
    ...
    # NOTE: no conversion performed here. A background worker should:
    #   - read saved_path (if present) or the provided markdown
    #   - generate a .docx at EXPORTS_DIR/<task_id>.docx
    status_payload = {
        "status": "pending",
        "task_id": str(task_id),
        "saved_input": saved_path,
        "status_url": reverse("md2docx:status", args=[task_id]),
        "download_url": reverse("md2docx:download", args=[task_id]),
    }
...
def status(request, task_id):
    ...
    """
    # If a result file exists, report 'finished'
